use ethers::{ 
    middleware::SingerMiddleware,
    signers::{LocalWallet, Signer},
    providers::{Http, Middleware, Provider, Ws},
    types:: {
        transaction::eip2718::TypedTransaction, BlockNumber, NameOrAddress, Transaction, TransactionRequest, H160, U256
    }, 
    utils::format_units
};
use std::{
...
type FlashbotsClient = FlashbotsMiddleware<SingerMiddleware<Provider<Ws>, LocalWallet>,LocalWallet>; 
...
impl EthClient{
   /// Initialize with automatic health checks 
    ...
    pub async fn create_flashbots_client (
        relay_singer: LocalWallet, 
        tx_signer: LocalWallet,
        ws_url: &str,
        relay_url: &str,
    ) -> Result<FlashbotsClient> {

        let provider = Provider::<Ws>::connect(ws_url).await?;
        let middleware = SingerMiddleware::new(provider, tx_signer);
        let flashbots = FlashbotsMiddleware::new(middleware, Url::parse(relay_url)?,  relay_singer);
    ...
    pub async fn send_private_tx(
        ...
        let pending_block = flashbots.inner().get_block_number().await?+1;
        let bundle = BundleRequest::new()
            .push_transaction(tx.clone().into_signed(flashbots.signer().as_ref())?)
            .set_block(pending_block.into());

       

        let simulation_result = flashbots.simulate_bundle(&bundle).await?;
        for tx_result in simulation_result.bundle_stats.results.iter(){
            if let Some(rever_reason) = &tx_result.revert {
                bail!("Transaction would revert: {:?}", rever_reason);
            }
        }

        let pending_bundle = flashbots.send_bundle(&bundle).await?;
        println!("Bundle sent: {:?}", pending_bundle);
        Ok(())
    }
